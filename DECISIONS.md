...
## 45. Fix: the aside (and "View Strategy-to-Execution Health Card") was hidden until an assessment loaded — even after #44 made the Control Plane link "persistent"

**Decision:** Restructured `index.html`: `#console-section`'s `hidden` toggle previously wrapped `.app-shell` (tab content **and** the aside together); it now applies only to the tab-content div (merged into `.shell-main`), while `.app-shell` — and the `<aside>` inside it — render unconditionally. The 5 assessment-dependent buttons (Export Markdown/Health Card/ADR, Print, **View Strategy-to-Execution Health Card**) are `disabled` by default in the markup and only enabled once `handleLoad()` succeeds, mirroring `dor-gatekeeper`'s existing "always-visible, individually-disabled" export-button convention (this app previously had no `disabled`-button convention at all — the whole aside was just hidden). `setAssessmentDependentButtonsEnabled()` re-disables them on Reset and on a failed load. Fixed two bugs this restructuring exposed (both invisible before, since the whole aside used to be hidden): (1) CSS Grid auto-placement flowed the aside into the wide `1fr` column instead of the narrow `300px` sidebar column whenever `.shell-main` was `[hidden]` and removed from grid flow — fixed with explicit `grid-column: 1` / `grid-column: 2` on `.shell-main`/`.app-aside`. (2) `#summary-bar` (inside the now-always-visible aside) kept showing the previous assessment's feature name/score after Reset or a failed reload, since nothing had ever cleared it independently of hiding the whole aside — fixed by explicitly resetting it to a `"Load an assessment above to enable exports."` placeholder in both paths. Separately, `button.secondary:disabled` had no override in this app's CSS — `.secondary` and `:disabled` have equal specificity, so a disabled secondary button (e.g. "View..." before load) kept `.secondary`'s enabled-looking transparent background; added `button.secondary:disabled` to fix it (same latent bug found and fixed in `dor-gatekeeper`, which had the identical CSS pattern and an already-shipped disabled-by-default secondary button — `DECISIONS.md #46` there).

**Why:** Reported directly — "View Strategy-to-Execution Health Card... shows up only when navigating from portal page." Root cause: the portal's deep link always auto-loads a sample via `applyDeepLink()` → `handleLoad()`, which reveals `#console-section` — masking that a direct visit to `index.html` (no portal, no data) left the *entire aside* hidden, not just this one button. That also meant #44's "persistent" Control Plane link commit didn't actually achieve persistence on a cold visit — it was still nested inside the same hidden wrapper. Fixing the aside's visibility properly (not just this one button) closes both gaps in the same change. Verified via headless browser end to end: a cold `index.html` visit shows the aside with the View button visible-but-disabled (and the Control Plane link visible-and-enabled); clicking a disabled button does nothing; loading a sample directly on this page (not via portal) enables the button and clicking it opens the preview; Reset disables the buttons again and restores the placeholder text (not stale data); the CSS grid keeps the aside pinned to the 300px column throughout; the portal's `?sample=&health-card=1#tab=` deep link and tab switching are unaffected. Zero console errors.

262 tests passing (unchanged — this is DOM/CSS behavior, covered by headless-browser verification rather than the Node test suite, consistent with how the rest of this app's DOM wiring is tested).

**Trade-off:** None — this closes a real gap in what #44 was already supposed to deliver, and the disabled-button convention it introduces is strictly more informative than the previous all-or-nothing hidden aside.
