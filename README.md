...
│   │   └── escalationTrend.js        # derives the "Predictive Escalation Alert" in-flight worsening flag from that history
│   ├── export/
...
- **Rework Risk & Remediation** — a severity-weighted score (High 10 / Med 5 / Low 2 points per open gap) classified into Low/Medium/High tiers with escalation guidance, plus a reference table of 3 remediation pathways — **Hard Reversion / Emergency Gate Review / Tech Debt Isolation** (`DECISIONS.md` #32) — reference only, never auto-selected. A Medium/High-tier load also writes a small diagnostic signal to same-origin `localStorage`, read by `dor-gatekeeper` as an advisory suggestion — see [Closed-loop tuning](#closed-loop-tuning-cross-app) below. Every load (any tier) also records this feature's score/tier to a same-browser history; if a later load of the *same* feature scores worse, a **"⚠ Predictive Escalation Alert"** appears on this panel naming the prior score and when it was last loaded — flagging an in-flight sprint collapse before the review happens, from real same-browser trend data, not a trained prediction (`DECISIONS.md` #35, renamed for clarity in #41). To see it working immediately, load the bundled **"Escalation Demo — Sprint 3 baseline"** sample, then **"Escalation Demo — Sprint 5 worsened"** — a matched before/after pair sharing one feature name, purpose-built to demonstrate the alert via the real mechanism, not fabricated data (`DECISIONS.md` #37).
- **Baseline Drift** — the State Sync Bridge's receiving half: paste/upload a `dor-gatekeeper` "Baseline" export from earlier alongside the currently-loaded "current" assessment, and see what changed — New Since Baseline / Resolved Since Baseline / Severity Changed, matched by `gap_id`. On-demand, one-shot comparison, not a live sync (`DECISIONS.md` #30). Click **"Load Sample Baseline"** to try it immediately with a bundled before/after pair hand-verified to exercise all 3 categories at once (`DECISIONS.md` #39) — load **"Baseline Drift Demo — Current checkpoint"** from the sample dropdown above as Current, then Compare.

**Executive exports**
- **Markdown recovery brief** — total exposure, top exposure gaps, RAID rollups, auto-generated recovery plan.
- **Executive Strategy-to-Execution Health Card** — TOM Feasibility Score, Total Financial Exposure reframed as **Protected Capital** (`DECISIONS.md` #32), a **Compliance Coverage** line mapping this assessment's gaps to the NIST AI RMF 1.0 functions they touch (`DECISIONS.md` #43 — NIST-only, never OWASP; see below), Top 3 Root Causes of Operational Rework, and mapped governance interventions. Click **"Export Executive Health Card"** in the sidebar (after loading any assessment) to download the `.md` file *and* render an on-screen preview — or click **"View Strategy-to-Execution Health Card"** underneath it to open the same preview without downloading anything, for opening it "when required" (`DECISIONS.md` #40). No need to arrive via the portal's deep link either, which just automates the Export click. The preview persists across tabs by design (`DECISIONS.md` #22) and has its own **Print Health Card** button, so "Save as PDF" captures the Health Card, not the Recovery Plan panel — plus a **Close Preview** button to dismiss it without resetting the loaded assessment (`DECISIONS.md` #38).
